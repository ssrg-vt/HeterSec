#!/usr/bin/env bash
#
# This script generates Popcorn Linux VMs (both x86_64 and aarch64), it is a
# modified version from the syzkaller project.
#
# Usage: ./create-image.sh -a aarch64 -d buster -n arm
#    and ./create-image.sh -a x86_64 -d buster -n x86
#
...
DIR=chroot
PREINSTALL_PKGS=openssh-server,curl,tar,gcc,libc6-dev,time,strace,sudo,less,psmisc,selinux-utils,policycoreutils,checkpolicy,selinux-policy-default,firmware-atheros,debian-ports-archive-keyring,vim

# If ADD_PACKAGE is not defined as an external environment variable, use our default packages
if [ -z ${ADD_PACKAGE+x} ]; then
    ADD_PACKAGE="make,sysbench,git,tmux,usbutils,tcpdump"
fi
...
display_help() {
    ...
    echo "   -n, --hostname		Set host name of this VM"
    echo
}
...
# Copy the VM image and ssh-key to a folder
mkdir -p $HOSTNAME.img
mv $RELEASE.img $HOSTNAME.img
mv $RELEASE.id_rsa $HOSTNAME.img
mv $RELEASE.id_rsa.pub $HOSTNAME.img
