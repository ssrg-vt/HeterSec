    ...
    ngx_log_t *log)
{
    ...
    of->fs_size = ngx_file_fs_size(&fi);
    of->is_dir = ngx_is_dir(&fi);
    of->is_file = 1;//ngx_is_file(&fi);
    of->is_link = ngx_is_link(&fi);
    of->is_exec = ngx_is_exec(&fi);

    //ngx_log_error(NGX_LOG_ALERT, log, ngx_errno, ngx_fd_info_n "NAME: \"%s\" , STAT FD: \"%d\" , Mode:\"%d\" , IS DIRECTORY: \"%d\" , SIZE: \"%ld\"" ,name->data, of->fd, fi.st_mode, of->is_dir, of->size);
    return NGX_OK;
}
